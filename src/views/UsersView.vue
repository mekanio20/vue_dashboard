    ...
    methods: {
        ...
        async allUsers(page) {
            try {
                ...
                const response = await this.$appAxios.get(`/user/all?status=all&page=${page}`, axiosConfig)
        ...
        async updateUser() {
            try {
                ...
                this.$appAxios.put('/admin/update/user', updateUser, axiosConfig)
                    .then((res) => {
        ...
        async deleteUser(id) {
            try {
                if (confirm('Bu maglumaty pozmak isleýärsiňizmi?')) {
                    const axiosConfig = { headers: { 'Authorization': `${localStorage.getItem('Authorization')}` } }
                    this.$appAxios.delete(`/admin/delete/user/${id}`, axiosConfig)
                        .then((res) => {
                            if (res.data.type === 'error') {
                                this.errorMessage(res.data.msg)
                            } else {
